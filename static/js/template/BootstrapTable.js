var bootstrapTableTemplate = 
`
<div class="bootstrap-table">
    <div id="menu" @mouseout='aaa'>
        <table border="1" width="100%" height="100%"  style="border:thin" cellspacing="0">
            <tr>
                <td id="mf1"  @mouseover = "over('1')" @mouseout="out(1)" style="cursor:default;border:outset 1;" align="center" onclick="parent.create()">
                    creat row
                </td>
            </tr>
            <tr>
                <td id="mf2"  @mouseover = "over('2')"  @mouseout="out(2)" style="cursor:default;border:outset 1;" align="center" onclick="parent.update();">
                    modify row
                </td>
            </tr>
            <tr>
                <td id="mf3"  @mouseover = "over('3')" @mouseout="out(3)" style="cursor:default;border:outset 1;" align="center" onclick="parent.del();">
                    delete row
                </td>
            </tr>
        </table>
    ...
    <div class="fixed-table-toolbar">
        ...
        :class="['columns-'+local.options.buttonsAlign, 'pull-'+local.options.buttonsAlign]">
            <button v-if="local.options.showPaginationSwitch"
                ...
                <i :class="[local.options.iconsPrefix, paginationSwitchIcon1, paginationSwitchIcon2]"></i>
            </button>
            <button v-if="local.options.showRefresh"
                ...
                @click="refresh">
                <i :class="[local.options.iconsPrefix, local.options.icons.refresh1, local.options.icons.refresh2]"></i>
            </button>
            <button v-if="local.options.showToggle"
                ...
                @click="toggleView">
                <i :class="[local.options.iconsPrefix, local.options.icons.toggle1, local.options.icons.toggle2]"></i>
            </button>
            <div v-if="local.options.showColumns" class="keep-open btn-group"
                ...
                <button type="button" data-toggle="dropdown"
                    ...
                    <i :class="[local.options.iconsPrefix, local.options.icons.columns1, local.options.icons.columns2]"></i>
    ...
    <div class="fixed-table-container"
        ...
        <div class="fixed-table-body">
            ...
            <table :class="local.options.classes">
                ...
                <tbody id= "tableq"  @mouseover='' @contextmenu="tbodyContextmenu()">
                    <!-- v-bind:class="class" -->
                    <tr v-for="(item, i) in renderData"
                        ...
                        <td>
                            <a class="detail-icon" href="javascript:">
                                <i :class="[local.options.iconsPrefix, icons.detailOpen1, icons.detailOpen2]"></i>
                            </a>
                        </td>
                        </template>
                            ...
                            <template v-for="(field, j) in header.fields">
                                <template v-if="!(!fieldColumns[j].visible || local.options.cardView && !fieldColumns[j].cardVisible)">
                                    ...
                                    <td v-else
                                        ...
                                        @blur="onTdCheckItemChange(item, $event)"
...
//待优化
function over(n){
    document.getElementById("mf"+n).style.backgroundColor="#ccc";
}
function out(n){
    document.getElementById("mf"+n).style.backgroundColor="white";
...
let DEFAULTS = {
    ...
    icons: {
        paginationSwitchDown1: 'glyphicon-collapse-down',
        paginationSwitchDown2: 'icon-chevron-down',
        paginationSwitchUp1: 'glyphicon-collapse-up',
        paginationSwitchUp2: 'icon-chevron-up',
        refresh1: 'glyphicon-refresh',
        refresh2: 'icon-refresh',
        toggle1: 'glyphicon-list-alt',
        toggle2: 'icon-list-alt',
        columns1: 'glyphicon-th',
        columns2: 'icon-th',
        detailOpen1: 'glyphicon-plus',
        detailOpen2: 'icon-plus',
        detailClose1: 'glyphicon-minus', 
        detailClose2: 'icon-minus'
    },
...
var BootstrapTable =
{
    ...
    watch: {
        my_columns (val) {
            ...
            if(vm.my_columns){
                console.log(vm.my_columns.slice())
                vm.local.columns = vm.my_columns.slice()
                Vue.set(vm.local,'columns',vm.my_columns.slice());
            }
    ...
    created: function () {
        let vm = this;
        var a = $.extend({}, DEFAULTS, vm.my_options)
        console.log()
        Vue.set(vm.local,'options',a);
        ...
        if(vm.my_data){
            Vue.set(vm.local,'data',vm.my_data);
        }
    ...
    computed: {
        paginationSwitchIcon1: function () {
            return this.local.options.icons[this.local.options.pagination
                ? 'paginationSwitchDown1' : 'paginationSwitchUp1'];
        },
        paginationSwitchIcon2: function () {
            return this.local.options.icons[this.local.options.pagination
                ? 'paginationSwitchDown2' : 'paginationSwitchUp2'];
        },
    ...
    methods: {
        ...
        initTable: function () {
            ...
            if (!Array.isArray(this.local.columns[0])) {
                Vue.set(this.local,'columns',[this.local.columns]);
            }
            ...
            this.local.columns.forEach(function (_columns, i) {
                _columns.forEach(function (column, j) {
                    ...
                    Vue.set(that.local.columns[i],j,column);
                });
            });
            for(let k in columns){
                Vue.set(this.fieldColumns,k,columns[k]);
            }
        ...
        onTdCheckItemChange(item,e){
            console.log("wukequnilllll")
            console.log(item)
            console.log(e)
            e.path[0].contentEditable=false
        },
            ...
            if(vm.local.data && row && row[id]){
                for(let index in vm.local.data){
                    if (vm.local.data[index][id] === row[id]){
                        Vue.set(vm.local.data,index,row);
                        break;
                    }
                }
            }
...
var BootstrapTable1 = Vue.extend(BootstrapTable)

Vue.component('bootstrap-table', BootstrapTable1)
