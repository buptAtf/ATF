...
//删除控件类型
            delClass: function () {
                var selectedTr = $("#classSelect").find("option:selected");
                    id = selectedTr.val();
                var that = this;
                if (id === undefined) {
                    $('#selectAlertModal').modal();
...
//删除方法
            delMethod: function (e) {
                var selectedTr = $("#methodSelect").find("option:selected");
                id = selectedTr.val();
                var that = this;
                if (id === undefined) {
                    $('#selectAlertModal').modal();
            ...
            methodClick: function () {
                ...
                app.methodId = $("#methodSelect").find("option:selected").val();
                var i = $("#methodSelect").find("option:selected").attr("name");
...
var setting1 = {
    ...
    callback: {
        ...
        onClick: function (event, treeId, treeNode, clickFlag) {
            ...
            $("#chosenStruct").val(treeNode.name);
            //查询class
            $.ajax({
                url: address3 + 'arcClass/queryArcVisibleOmClasses',
                type: 'post',
            ...
            // $("#classSelect").children().eq(0).prop("selected","selected");
            //  console.log($("#classSelect").find("option").first().attr("id"));
            //查询抽象架构
            // $.ajax({
            //     url: address+'/abstractarchitectureController/abstractarchitecturequery',
            //     type: 'post',
